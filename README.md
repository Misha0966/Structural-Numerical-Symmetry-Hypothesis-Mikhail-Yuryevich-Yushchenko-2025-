...
Может совпадать как начальные так конечные цифры, при этом  имеют разные разрядности для этих чисел.

Может наблюдаеться частичная симметрия: совпадают начальные или конечные цифры PQ и N * k 
...
Умножение каждой части на натуральное число k
...
Для любого целого числа N≥10, если его разделить на m≥2 частей, максимально близких по длине, умножить каждую часть на одно и то же натуральное число k, а затем объединить результаты как десятичное число PQ, то:

Возможно полное совпадение: PQ = N * k

Может совпадать как начало, так и конец, при этом числа PQ и N * k имеют разные разрядности.

Может наблюдаеться частичная симметрия: совпадает начало или конец.
...
### 3.2 Анализ зависимости от множителя k
...
|--------|-----|--------------|------------|---------------|
|899766  |   4 |   35963064   |   3599064  |     3 и 4     |
|   17   |   4 |     428      |     68     |       8       |
|  1234  |   4 |     48136    |    4936    |     4 и  6    |
...
| 11  |  7  |      [1][1]    |     77     |   77   |     Полное совпадение    |
...
| `compare_pq_nk`              | Сравнивает PQ и N * k и определяет тип совпадения|
| ` check_hypothesis`          | Проверяет гипотезу для одного числа N            |
| ` run_tests`                 |  Запускает проверку гипотезы для диапазона чисел |
...
### 4. Интеграция в математическое ПО (например, Julia, Mathematica, Python)
